...
def chat(req: ChatRequest):
    ...
    context = "\n\n---\n\n".join([f"[{r['section']}] {r['text']}" for r in retrieved]) if retrieved else "NO_RELEVANT_CONTEXT_FOUND"

    convo_text = mem.formatted()
    try:
        answer = llm.chat_with_context(context=context, conversation=convo_text, user_message=req.message)
    except Exception as e:
